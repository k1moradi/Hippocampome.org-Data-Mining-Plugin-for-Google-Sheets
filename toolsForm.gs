...
function refHighlights(input) {
  ...
  var units = "(?:°C?|m(?:sec|[lLMVs])(?:[ \\\/]min)?(?:⁻¹)?|min(?:utes?)?|(?!nAChR*)[pn][ASF]|mS(?:[·\\\/]cm⁻²)?|%|percent|times?|mega*ohms*|(?:(?:representative|recorded|morphological(?:ly)?|physiological(?:ly)?|identified|and|or) )*(?:cells*|(?:inter)?neurons*|pairs*|sets?|PII)|quantal*|stimul(?:us|i)|(?:synaptic )?connections*|[µμ].?(?:ec)?|[kKM]?Hz|.?Ω|(?:(?:consecutive|individual|single|continuous|successive|superimposed))*(?: (?:response|sweep|event|trace|record(?:ing)?|observation|trial|[esmuESMU]?\\.?[eiEI]\\.?[Pp]\\.?[Ss]\\.?[CPcp])[Ss]?))";
